...
namespace Student_Project
{
    public partial class UpdateQuestionform : Form
    {
        ...
        private void cmbSet_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                cmbquestionNo.Items.Add(ds.Tables[0].Rows[i][0].ToString());
            }

        }

        private void cmbquestionNo_SelectedIndexChanged(object sender, EventArgs e)
        {
            DataSet ds = Connection.GetData("Select question, optionA, optionB, optionC, optionD, ans from et_questions where qset = '" + cmbSet.Text + "' and qno = '" + cmbquestionNo.Text + "' ");
        ...
        {
            if (cmbquestionNo.SelectedIndex != -1)
            {
                String qset = cmbSet.Text;
                String qNo = cmbquestionNo.Text;
